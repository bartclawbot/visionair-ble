...
## #21 SETTINGS byte pairs investigation

### Question

What do SETTINGS bytes 9-10 and byte 7 actually encode?
The library has `SETTINGS_BYTE_PAIRS` with three byte pairs extracted from
early captures: LOW=(0x19, 0x0A), MEDIUM=(0x28, 0x15), HIGH=(0x07, 0x30).
...
**The byte pairs were clock sync artifacts.** The pairs from early captures
(0x19/0x0A = min 25/sec 10, etc.) are plausible minute:second values.
...
- `SETTINGS_BYTE_PAIRS` constant: contains clock sync artifacts, not confirmed config values
...
- Added investigation comments to `SETTINGS_BYTE_PAIRS`, `SettingsMode`, and
